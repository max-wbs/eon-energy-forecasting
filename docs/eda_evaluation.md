...
Notebook `01_delivered_data.ipynb` (zwei Abbildungen), die gerenderten
...
Takeaway-Texten übernommen. Die frühere Evaluation bezog sich auf die alte
Notebook-Aufteilung (frühere Fassung von `03_eda` plus `03b`, beide durch das
konsolidierte `03_eda.ipynb` ersetzt) und ist hiermit vollständig ersetzt —
Achtung: einzelne Kennzahlen haben sich durch die Konsolidierung geändert
(Abschnitt 5)._
...
| `01_delivered_daily_active_households` | Tagesabdeckung schwankt 25–67 Haushalte, Peak 67, Einbruch 2021. | **aufnehmen, fällt zuerst** | Ergänzt die Coverage-Zeilen um die aggregierte Lesart; bei knappem Budget trägt ein Satz („never more than 67 of 156 delivering simultaneously") den Befund fast vollständig. |
| §1 `03_eda_load_seasonality` | Saisonhub Faktor 3,2 (DJF 48,4 vs. JJA 15,3 kWh/Tag), Peak-Tagesmittel 70,4 kWh (14.02.2021), wiederkehrende Kältespitzen. | **mit Änderung** | Zeigt die Dynamik, die die Beschaffung tragen muss, inkl. Wiederholbarkeit der Spitzen. Kompositions-Caveat (25–67 aktive Haushalte/Tag) steht im Notebook-Fließtext und muss in die Caption; die Takeaway-Zahl „~11 kWh im Sommer" ist das Rolling-Minimum, nicht das JJA-Mittel (15,3) — präzisieren. |
...
- **§3.1 / §6:** „rises to ~68 kWh/day at −15 °C" ersetzen durch „rises to
  ~70 kWh/day in the coldest well-populated bins (≈ −9 °C), ~4–5× the summer
  plateau" — die n≥30-Kurve deckt −15 °C nicht. Kappungshinweis (0,42 % >
  130 kWh, Max 250) aus dem Zellen-Output in die Caption übernehmen.
- **§1:** Caption-Caveat Komposition: „fleet mean over a changing subset
  (25–67 active households per day, see coverage figure); the seasonal shape,
  not year-to-year levels, is the robust finding." Takeaway präzisieren:
  Rolling-Minimum ~12 kWh vs. JJA-Mittel 15,3 — eine der beiden Zahlen
  wählen und benennen.
...
- **Lag-Features / Persistenz-Baselines (Level 0, Notebook 07):**
  ...
  Autoregression nötig ist (Niveauspreizung Faktor ~5, die ein gepooltes
...
- Household levels spread by a factor of ~5 and daily draw is highly
...
1. **„−15 °C"-Behauptung** in §3.1-Takeaway *und* §6-Key-Findings nicht durch
   ...
   (Abschnitt 3).
2. **Zahlen-Drift durch die Konsolidierung:** Die früheren Fassungen filterten
   auf ≥ 60 Liefertage (136 Haushalte); konsolidiert gilt `min_days = 0`
   (alle 153). Dadurch: Temperaturresponse-Median **−0,84 statt −0,86**,
   Anteil < −0,7 **78 % statt 82 %**, CV-Mediane **0,60/0,45 statt 0,61/0,47**.
   Ältere Reportentwürfe, Folien oder Notizen, die noch die alten Zahlen
   zitieren, müssen auf den konsolidierten Stand gezogen werden — die alten
   Notebooks und Begleitdocs sind gelöscht und nicht mehr Referenz.
...
5. **§2.2-Output „factor 14":** Extremwert-Framing (6,5 → 93,7); im Report
   durchgängig p5–p95 (Faktor ~5) verwenden, sonst widersprechen sich §2.1
   und §2.2 scheinbar.
6. **Sommer-Niveau in §1:** „~11 kWh/day" (Rolling-Minimum) vs. 15,3 kWh/Tag
   (JJA-Mittel) — im Report eine Größe wählen und benennen.
